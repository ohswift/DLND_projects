...
def token_lookup():
    ...
    # TODO: Implement Function
    keys = set(['.', ',', '"', ';', '!', '?', '(', ')', '--', '\n'])
    values = ['Period', 'Comma', 'Quotation', 'Semicolon', 'Exclamation', 'Question', 'LeftParentheses',
              'RightParentheses', 'Dash', 'Return']
...
def get_batches(int_text, batch_size, seq_length):
    ...
    n_batches = len(arr)//n_per_batch
    
    arr = arr[:n_batches * n_per_batch]
    ...
    for n in range(0, arr.shape[1], seq_length):
        ...
        batachs.append([list(x), list(y)])
        
    return np.array(batachs)
